 ...
 */
export const comparePollMessage = async(
	options: string[],
	pollOptionHashes: string[]
): Promise<string[]> => {
	const selectedOptions: string[] = []
	const crypto = getCrypto()
	for(const option of options) {
		const hash = Buffer.from(
			await crypto.subtle.digest('SHA-256', new TextEncoder().encode(option))
		)
			.toString('hex')
			.toUpperCase()
...
export const decryptPollMessageRaw = async(
	...
	const decryptedMessage = await decryptPollMessageInternal(
		encPayload,
		encIv,
		additionalData,
		decryptionKey
